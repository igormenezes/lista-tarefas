...
RSpec.describe Task, type: :model do
	...
	it 'show tasks by list and user' do
		...
		task.save
		task = Task.new(description: 'teste2', active: 1, list_id: list.id)
		task.save

		list2 = List.new(name: 'teste2', available: 1, active: 1, user_id: @user.id)
		list2.save

		task = Task.new(description: 'teste2', active: 1, list_id: list2.id)
		task.save

		list3 = List.new(name: 'teste3', available: 1, active: 1, user_id: user.id)
		list3.save
		
		task = Task.new(description: 'teste3', active: 1, list_id: list3.id)
		task.save

		tasks = Task.select(:description, :active)
